...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="px-6 py-8">
      ...
      {activeTab === "overview" ? <OverviewTab /> : <AuditTab />}
    </div>
  );
}
    ...
    <div>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="text-sm text-teal-700 cursor-pointer hover:underline">
            View full log →
          </span>
        </div>
  ...
  useEffect(() => {
    async function loadLogs() {
      try {
        ...
        if (filters.type !== "all") query = query.eq("event_type", filters.type);
          ...
          {!loading &&
            logs.map((row, i) => (
              <tr key={i} className="hover:bg-slate-50/60">
                ...
                <td className="px-4 py-3">{row.action ?? "—"}</td>
                <td className="px-4 py-3">{row.field_changed ?? "—"}</td>
                <td className="px-4 py-3 text-rose-600 text-xs">{row.old_value ?? "—"}</td>
                <td className="px-4 py-3 text-emerald-600 text-xs">{row.new_value ?? "—"}</td>
                <td className="px-4 py-3">{row.candidate_name ?? "—"}</td>
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
